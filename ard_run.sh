...
do
	...
	awk -F "\t" 'BEGIN{OFS="\t"}{
		print $6, "Resfinder", "ARD", $7, $7, ".", "strand", "frame",
...
awk -F "\t" 'BEGIN{OFS="\t"}{ print $1, "Victors", "ARD", $6, $7, $10, "strand",
	"frame", "hit_id=", $2, ";hit_name=", $11}' "$outdir"/vfdb/vfdb_"$infile"_90.tsv > "$outdir"/vfdb/almost.gff
"$path_to_python3" gffmaker.py \
	"$outdir"/victors/almost.gff \
	"$path_to_gffs"/"$sample"_contigs.gff \ ####
	blast \
	> "$outdir"/victors/"$infile"_victors.gff
...
awk -F "\t" '{ if(($4 >= 90) && ($5 >= 90)) {
	print } }' "$outdir"/vfdb/vfdb_"$infile".out > "$outdir"/vfdb/vfdb_"$infile"_90.tsv
awk -F "\t" 'BEGIN{OFS="\t"}{ print $1, "VFDB", "Virulence Factors", $6, $7, $10, "strand",
	"frame", "hit_id=", $2, ";hit_name=", $11}' "$outdir"/vfdb/vfdb_"$infile"_90.tsv > "$outdir"/vfdb/almost.gff
"$path_to_python3" gffmaker.py \
	"$outdir"/vfdb/almost.gff \
	"$path_to_gffs"/"$sample"_contigs.gff \ ####
	blast \
	> "$outdir"/vfdb/"$infile"_vfdb.gff
...
awk -F "\t" 'BEGIN{OFS="\t"}{ print $1, "RGI", "ARD", "start", "end", ".", "strand",
	"frame", "hit_name=", $1, ";ARO_accession=", $11, ";drug_class=", $15,
	";gene_family=", $17}' "$outdir"/rgi/rgi_"$infile"_nosemi.txt > "$outdir"/rgi/almost.gff
"$path_to_python3" gffmaker.py \
	"$outdir"/rgi/almost.gff \
	"$path_to_gffs"/"$sample"_contigs.gff \ ####
	rgi \
	> "$outdir"/rgi/"$infile"_rgi.gff
